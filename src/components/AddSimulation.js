...
const AddSimulation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    //endpoint
    const createEndpoint = `${process.env.REACT_APP_API_URL}${process.env.REACT_APP_SIMULATIONS_ENDPOINT}`;
    ...
    try {
      ...
      console.log('Simulation ajouté avec succès', response.data);
      setStep(3); // Afficher le résumé
      ...
      setErrorMessage('Erreur lors de la création de la simulation');
    }
    finally {
        ...
        <div className="p-4">
          <div className="flex justify-center">
            ...
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
                <h2 className="text-2xl font-bold mb-6 text-center">Demander une simulation</h2>
                
                {/* Progress bar */}
                <div className="flex justify-between mb-8">
                  <div className={`w-1/3 h-3 rounded-md transition-all ${step >= 1 ? 'bg-green-600' : 'bg-gray-300'} mr-3`} />
                  <div className={`w-1/3 h-3 rounded-md transition-all ${step >= 2 ? 'bg-green-600' : 'bg-gray-300'} mr-2`} />
                  <div className={`w-1/3 h-3 rounded-md transition-all ${step >= 3 ? 'bg-green-600' : 'bg-gray-300'} mr`} />
                </div>
