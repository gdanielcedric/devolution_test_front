...
const TableSimulation = () => {
  ...
  const fetchData = async (token) => {
    ...
    //endpoint
    const listEndpoint = `${process.env.REACT_APP_API_URL}${process.env.REACT_APP_SIMULATIONS_ENDPOINT}/all`;

    try {
      const response = await axios.post(listEndpoint, dataSend, config); // Remplacer par ton endpoint
      console.log('Data à afficher (Simulations)', response);
  ...
  // Export en CSV
  const exportCSV = () => {
    const csvRows = data.map(item => `${item.quoteReference},${item.immatriculationNumber},${item.assurProduct},${item.price},${item.createdAt},${item.endDate},${item.status}`).join("\n");
    const blob = new Blob([csvRows], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.setAttribute('href', url);
    a.setAttribute('download', 'export_simulations.csv');
    a.click();
  };

  // Export en Excel
  const exportExcel = () => {
    const filteredForExcel = data.map(({ quoteReference,immatriculationNumber,assurProduct, price, createdAt, endDate, status}) => ({
      quoteReference,
    ...
    XLSX.writeFile(wb, "export_simulations.xlsx");
  };
        ...
        <table className="min-w-full table-auto">
          <thead>
            <tr className="bg-gray-800 text-white">
              ...
              <th className="px-3 py-2">Date Echéance</th>
