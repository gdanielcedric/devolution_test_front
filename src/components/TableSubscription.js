...
const TableSubscription = () => {
  ...
  // Export en CSV
  const exportCSV = () => {
    const csvRows = data.map(item => `${item.quoteReference},${item.suscriberNom},${item.suscriberPhone},${item.immatriculationNumber},${item.assurProduct},${item.price},${item.createdAt},${item.endDate},${item.status}`).join("\n");
  ...
  // Export en Excel
  const exportExcel = () => {
    const filteredForExcel = data.map(({ quoteReference, suscriberNom, suscriberPhone, immatriculationNumber,assurProduct, price, createdAt, endDate, status}) => ({
      quoteReference,
      suscriberNom,
      suscriberPhone,
